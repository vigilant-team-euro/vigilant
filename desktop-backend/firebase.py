...
def sendData(branch_name:str, result:list):
    ...
    db.collection("branches").document("branch1").collection(frame_name).document("datetime").set({"datetime": now_str})
    ...
    for person in result:
        ...
        db.collection("branches").document("branch1").collection(frame_name).document(person_name).set(data)
        person_count += 1